          ...
          {colors.map((color) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => {
                  const hex = typeof color.value === 'string' ? color.value : String(color.value)
              ...
              >
                <div
                  className={`w-12 h-12 rounded-full border-2 ${isSelected ? 'border-black' : 'border-transparent'} group-hover:border-black/25 transition-colors`}
                  style={{ backgroundColor: color.value }}
                />
                {isSelected && (
                  <div className="absolute top-0 left-0 right-0 bottom-4 flex items-center justify-center pointer-events-none">
                    {/* simple check glyph — robust across environments */}
