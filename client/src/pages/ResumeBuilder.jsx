...
// import { ArrowLeftIcon, Briefcase, ChevronFirst, ChevronRight, DownloadIcon, EyeIcon, EyeOffIcon, FileText, FolderIcon, GraduationCap, Share2, Share2Icon, Sparkles, User} from 'lucide-react'
...
//               <hr className='absolute top-0 left-0 h-1 bg-gradient-to-r form-green-500 to-green-600 border-none transition-all duration-2000' style={{width:`${activeSectionIndex * 100 / (sections.length - 1)}%`}}/>
...
//                       <ChevronFirst className='size-4'/>Previous
//                     </button>
//                   )}
...
//                 <button onClick={changeResumeVisibility} className="flex items-center p-2 px-4 gap-2 text-xs bg-gradient-to-br from-purple-100 to-gray-200 text-purple-700 rounded-lg ring-purple-300 hover:ring transition-colors">
//                   {resumeData.public ? (
...
const ResumeBuilder = () => {
  ...
  //
  // useEffect(() => {
  //   console.log('resumeData.accent_color ->', resumeData.accent_color)
  ...
  // Keep a CSS variable updated so child preview components can pick it up reliably.
  useEffect(() => {
    if (resumeData.accent_color) {
      document.documentElement.style.setProperty('--resume-accent', resumeData.accent_color)
    }
  }, [resumeData.accent_color])
  ...
  const saveResume = async () => {
    try {
      ...
      // remove image form updatedResumeData (avoid sending raw File in JSON)
      if (typeof resumeData.personal_info.image === 'object') {
        delete updatedResumeData.personal_info.image
      }
            ...
            <div className="relative w-full">
              <div className="absolute bottom-3 left-0 right-0 flex items-center justify-end gap-2">
                ...
                <button
                  ...
                  className="flex items-center p-2 px-4 gap-2 text-xs bg-gradient-to-br from-purple-100 to-gray-200 text-purple-700 rounded-lg ring-purple-300 hover:ring transition-colors"
                >
